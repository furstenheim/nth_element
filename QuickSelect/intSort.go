...
// s := // some slice
// QuickSelect.IntBuckets(nthElementUtils.IntSorter(s), 5)
...
func IntSelect(array nthElementUtils.IntSorter, k, left, right int) {
	for true {
		...
		pivotIndex = Intpartition(array, pivotIndex, left, right)
		if k == pivotIndex {
			return
...
func Intpartition (array nthElementUtils.IntSorter, pivot, left, right int) int {
